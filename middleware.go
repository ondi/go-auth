...
type TokenFinder interface {
	TokenFind(r *http.Request) (out []Token)
}
...
type StatusBodyError_t struct {
	...
	Failed    []StatusBodyToken_t `json:"failed,omitzero"`
	Passed    []StatusBodyToken_t `json:"passed,omitzero"`
	IsDebug   bool                `json:"-"`
}
...
func (self StatusBody_t) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	self.Error.Debug = fmt.Sprintf("tokens: passed=%v, failed=%v", len(found.Passed), len(found.Failed))
	if self.Error.IsDebug {
		self.Error.Passed = nil
		for _, v := range found.Passed {
			self.Error.Passed = append(self.Error.Passed, StatusBodyToken_t{
				Token: fmt.Sprintf("%.128s", v.GetValue()),
				Error: fmt.Sprintf("%v", v.GetError()),
			})
		}
		self.Error.Failed = nil
		for _, v := range found.Failed {
			self.Error.Failed = append(self.Error.Failed, StatusBodyToken_t{
				Token: fmt.Sprintf("%.128s", v.GetValue()),
				Error: fmt.Sprintf("%v", v.GetError()),
			})
		}
