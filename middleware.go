...
func (self *Status_t) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if self.debug {
		...
		res := debug_body_t{
			Error: "token debug report",
			Total: found.KeysFound,
		}
		for _, v := range found.Passed {
			res.Passed = append(res.Passed, debug_token_t{Token: fmt.Sprintf("%.64s", v.GetValue()), Error: fmt.Sprintf("%v", v.GetError())})
		}
		for _, v := range found.Failed {
			res.Failed = append(res.Failed, debug_token_t{Token: fmt.Sprintf("%.64s", v.GetValue()), Error: fmt.Sprintf("%v", v.GetError())})
		}
