...
type debug_token_t struct {
	Token string `json:"token"`
	Error string `json:"error"`
}

type debug_body_t struct {
	Code    int             `json:"code"`
	Message string          `json:"message"`
	Found   int             `json:"found"`
	Failed  []debug_token_t `json:"failed"`
	Passed  []debug_token_t `json:"passed"`
}

type debug_root_t struct {
	DebugBody debug_body_t `json:"error"`
}
...
func (self *Status_t) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if self.debug {
		...
		res := debug_root_t{
			DebugBody: debug_body_t{
				Code:    self.status_code,
				Message: fmt.Sprintf("tokens: found=%v, passed=%v, failed=%v", found.KeysFound, len(found.Passed), len(found.Failed)),
				Found:   found.KeysFound,
			},
		}
		for _, v := range found.Passed {
			res.DebugBody.Passed = append(res.DebugBody.Passed, debug_token_t{Token: fmt.Sprintf("%.64s", v.GetValue()), Error: fmt.Sprintf("%v", v.GetError())})
		}
		for _, v := range found.Failed {
			res.DebugBody.Failed = append(res.DebugBody.Failed, debug_token_t{Token: fmt.Sprintf("%.64s", v.GetValue()), Error: fmt.Sprintf("%v", v.GetError())})
		}
